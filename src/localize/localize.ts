...
export function localize(string: string, search = '', replace = ''): string {
  ...
  try {
    const langObj = languages[advancedCameraCardLanguage ?? DEFAULT_LANG];
    if (langObj) {
      translated = string.split('.').reduce((o, i) => o?.[i], langObj);
    }
  ...
  // Fallback to original string if translation is still not a string
  if (!translated || typeof translated !== 'string') {
    translated = string;
  }
